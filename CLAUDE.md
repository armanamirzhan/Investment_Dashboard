...
[.github/workflows/refresh-section.yml](.github/workflows/refresh-section.yml) is a manual on-demand workflow triggered by buttons in the per-company report HTML. Inputs: `ticker` (string) and `section` (choice: thesis / catalysts / risks / sources). It runs an Opus 4.7 agent that researches and updates the single specified field on the company entry plus a `{section}_updated` timestamp, then `regenerate_all()` + deploy + commit + Pages trigger. The dispatch happens from the browser via the GitHub REST API using a fine-grained PAT the user pastes into a modal (stored in localStorage as `gh_pat_v1`). Same machinery powers the dashboard's "Refresh All Metrics" button, which dispatches the existing `refresh.yml`. The shared client UI (CSS, JS, button helpers) lives in [scripts/refresh_ui.py](scripts/refresh_ui.py) and is imported by both `generate_dashboard.py` and `generate_reports.py`.
